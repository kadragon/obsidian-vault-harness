...
#   `10_Areas/과업심의/`는 제외한다 (2026-08-02 사용자 결정): 이 폴더의 `심의의견_*.md`·
#   `*_사업목록.md` 등은 회차마다 재생산되는 **심의 서식**(`사업 개요`/`과업내용 적정성` 구조)이지
#   업무사안이 아니다. 제외 전 실측으로 미통과 20건 중 15건이 이 경로였고, 회차가 열릴 때마다
#   같은 경고가 재발했다. 제외 후 잔여 5건은 진짜 구형 분석 노트로 계속 검출된다.
#   기준 동기화 대상: docs/eval-criteria.md → Template Adherence
if ("/10_Areas/" in fp_norm and note_type == "work"
        and "/10_Areas/과업심의/" not in fp_norm):
    # 앵커 비교는 두 단계다:
