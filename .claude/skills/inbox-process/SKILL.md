...
오케스트레이터의 책임은 스캔, 루트 triage, 에이전트 디스패치, 사용자 확인, 삭제 일괄 처리, 최종 보고다. 파일 내용 Read·노트 작성은 하지 않는다.
...
두 서브에이전트의 결과가 도착하면, 성공적으로 처리된 원본 파일을 **즉시 삭제**하고 최종 보고한다. 사용자 승인을 기다리지 않는다.
