...
triage의 `classify_pdf.py` 호출은 분류 판단용이다. 본문 추출·노트 작성은 워커가 한다. 워커가 같은 파일을 다시 분류해도 추출본 경로가 해시 기반이라 재사용된다.
...
**선례 전달 규칙**: 선례 노트는 최대 1건, 용도는 사실 참조(근거법령·담당자·네이밍·반복 주기)로 명시한다. "선례 섹션 구조를 그대로 승계하라"고 쓰지 않는다 — 구조 권위는 `99_Template/` + `docs/eval-criteria.md`다. 워커 프롬프트에 다음 한 줄을 넣는다:
>
> `구조는 99_Template/_업무사안.md를 따를 것. 선례는 사실(근거법령·담당자·네이밍)만 참조하고 섹션 구성은 베끼지 말 것.`
...
처리 단위:
- {폴더 또는 파일 절대경로1} → area: 수업성적 / 유사 노트 후보: [[...]], [[...]] / 태그 힌트: #업무/수업성적
