...
- **유사 노트 힌트 (선택)**: 오케스트레이터가 미리 vault-navigator로 수집한 과거 노트 경로 + 태그 패턴. 없으면 직접 `vault-navigator`를 Agent 도구로 호출.
...
마지막에 삭제 권고 목록을 별도로 제시한다 — **실제 삭제는 하지 않는다** (오케스트레이터가 사용자 승인 후 일괄 처리):
...
- **태그 작성**: 노트 `## 관련` 섹션 태그는 직접 정하지 말고, 노트 생성 완료 후 `tag-validator` 에이전트(suggest 모드)에 위임한다.
...
- `vault-navigator` — 유사 과거 노트 탐색 (오케스트레이터가 미리 수집해 넘겨주면 생략).
- `tag-validator` — 생성한 노트의 태그 작성 위임.
- 문서에 에러 로그/개선 사항이 섞여 있으면 `## 열린 질문`에 "incident-analyst·improvement-planner 추가 호출 필요"를 명시하고 오케스트레이터 판단에 맡긴다.
