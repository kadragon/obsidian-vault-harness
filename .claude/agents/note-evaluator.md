...
3. **훅 무음이 통과가 아닌 잔여분을 직접 확인한다** (`eval-criteria.md` §기계 검사 커버리지 표 — 형식 검증기는 값이 아예 없으면 무음이다):
   ...
   - `20_Training/` 노트의 `#업무/` **부재** — Check 5 범위 밖이다(미보유 25/35 = 71%로 관행이 아니라 2026-08-02 제외). 교육 내용이 특정 업무에 귀속되는데 없으면 finding.
   - `#부서/` 부재는 **finding이 아니다** — 선택 필드로 강등됐다(2026-08-02, 실측 56/201 미보유). 지적하지 말 것.
   - 태그 **area 배정이 내용과 맞는지** (문맥 판단이라 기계가 못 한다).
   - 기준 5 위키 피드백: `python3 .claude/lib/moc_gate.py . --json`으로 임계 도달 도메인을 검출하고, 해당하면 MOC 존재 + **노트↔MOC 양방향 등록**을 grep으로 확인한다.
