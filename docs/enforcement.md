...
- **Check 4 — 필수 섹션 앵커 (`10_Areas/` + `type: work`, `10_Areas/과업심의/` 제외):** `## 관련`·`## 할 일` 중 하나라도 없으면 warn. **템플릿 문자열 일치나 5섹션 전부 존재는 검사하지 않는다** (2026-07-30). 별칭 허용은 **두 가지 서로 다른 장치**다 — 혼동하지 말 것:
