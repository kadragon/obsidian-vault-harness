...
`validate_tag.py`가 판정하는 것: 금지 `#업무/` 접두어 · 괄호·공백·`&`·`+` · 미등록 area(런타임에 `10_Areas/` 폴더명 **+ `_Wiki/topics/*-운영-MOC.md`** 합집합에서 유도 — 2026-08-25) · **직급 매핑**(`행정주사보`→`주무관`) · **부서명 매핑** · `P_` 접두어 · `퇴직/` 중간 경로 · 학과 조교 경로.
...
- **Check 2b — `status:` 필수 + enum 검증** (note-bearing 폴더): 없거나 어휘 밖이면 warn. 허용: `open|in-progress|hold|closed|active` (`_메타데이터 규칙.md` 5개 고정). `done`/`resolved`/`pending-action` 드리프트가 status-sync를 눈멀게 하던 것을 잡는다 (2026-06).
  - **심의 서식은 `status:` 부재만 면제한다** (2026-08-02 사용자 결정). 서식은 진행 상태를 갖는 업무사안이 아니라 회차마다 재생산되는 산출물이라 없는 게 정상이고, 실측 10건이 회차마다 같은 경고를 냈다(그 10건은 Check 5 미보유 10건과 **동일 집합**). 판별 키는 Check 4·5와 공유하는 `is_simui_form`(파일명 어휘) — 아래 Check 4 참조.
  - **다만 값이 *있으면* 어휘는 검증한다.** 통째로 끄면 서식 15건 중 status를 가진 5건의 드리프트가 무검사가 된다. **교훈: 제외 근거를 문장으로 정확히 읽을 것 — "상태 개념이 없다"는 부재를 정당화하지 "아무 값이나 된다"를 뜻하지 않는다. 근거보다 넓게 끄면 그 차이만큼이 새 무게이트다.**
...
- **Check 4 — 필수 섹션 앵커 (`10_Areas/` + `type: work`, `10_Areas/과업심의/`의 심의 서식 제외):** `## 관련`·`## 할 일` 중 하나라도 없으면 warn. **템플릿 문자열 일치나 5섹션 전부 존재는 검사하지 않는다** (2026-07-30). 별칭 허용은 **두 가지 서로 다른 장치**다 — 혼동하지 말 것:
  ...
  - **판별은 `is_simui_form` 한 곳에서 계산해 Check 2b·4·5가 공유한다** (2026-08-02). 초판은 Check 4 안에만 있었고, 그 결과 같은 서식이 status·`#업무/` 경고를 계속 냈다 — 제외 근거("업무사안이 아니다")는 세 검사에 똑같이 적용되는데 제외는 한 곳에만 걸려 있었다. **교훈: 제외 근거가 노트의 *종류*에 대한 주장이면, 그 종류를 전제하는 검사 전부에 걸어야 한다 — 한 검사에만 걸면 나머지가 같은 오탐을 계속 낸다.**
...
- **Check 5 — `#업무/` 구체 태그 존재 (`10_Areas/`+`type: work` · `14_Changes/`, 심의 서식 제외):** 없으면 warn (2026-07-30 신설, 2026-08-02 PR #20 리뷰로 `14_Changes` 확장). `validate-tags.sh`는 **발견한 태그의 형식**만 검증하므로 태그가 전무한 노트는 무음 통과한다 — `eval-criteria.md` 기준 2가 "tag missing entirely"를 1점으로 규정하는데도 기계 검사가 비어 있던 구멍이다.
  ...
  - **`20_Training/`은 제외한다** (2026-08-02 사용자 결정): 실측 미보유 25/35(71%)로 **관행이 아니다** — 기계화하면 다수가 오탐이다. 규칙은 실측 관행과 대조해 쓴다(AGENTS.md 위임 비용 규칙 #6).
    - **같은 결정으로 `99_Template/_교육.md`의 `- #업무/`도 제거했다**(2026-08-02, PR #21 리뷰 후속). 검사만 빼고 템플릿을 두면 "템플릿은 요구하는데 아무것도 강제하지 않는" 상태가 남고, 문서는 그 부재를 "평가자 판단"으로 넘기지만 `note-evaluator`는 `inbox-process` 5단계-3-b 조건에서만 호출돼 교육 노트는 그 경로를 거의 타지 않았다 — **실질 무게이트**. 템플릿을 관행에 정렬해 요구 자체를 없앴으므로 교육 노트의 `#업무/` 부재는 이제 **위반이 아니다**(별도 게이트 불필요). **교훈: 검사를 강등하면 그 기준을 요구하던 템플릿·문서도 같이 내려야 한다 — 안 그러면 "요구는 남고 집행만 사라진" 무게이트가 생긴다.**
  - **심의 서식도 제외한다** (2026-08-02): Check 2b와 같은 근거·같은 `is_simui_form` 키. 실측상 status 누락 10건과 `#업무/` 미보유 10건이 동일 집합이었다.
