...
2. **Follow templates** — new notes must use the matching template from `99_Template/`. Internal links use plain `[[노트명]]`; never `![[...]]` **노트** embeds unless explicitly requested (첨부 파일 임베드 — 이미지·PDF·hwp 등 — 는 예외, 2026-08-25). 기계 판정은 `.claude/lib/note_rules.py` Check 1.
